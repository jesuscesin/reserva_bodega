      ...
      if(count($asig)>0){//el array $haberes por lo menos siempre tendra el valor de cantidad de filas de la tabla haberes
			...
			if($existe_codigo == 0){
				$queryin = "INSERT INTO ZC0020(C0_SECUENCIA, C0_COD,C0_CAMARTICULO, C0_DESCR, C0_ORIGEN, C0_DESTINO, C0_CANTIDAD, C0_FTRASPASO, C0_FECHA, R_E_C_N_O_, D_E_L_E_T_,
								C0_USUARIO,C0_STATUS,C0_UM,C0_CODBAR,C0_CONV,C0_SEGUM,C0_RSECUENCIA,C0_CONTA,C0_GRUPO,C0_CUSTO1,C0_CUSTO5,C0_NUMSEQ,C0_CANT2UM, C0_CANTEN, C0_ESTADO, C0_RESERVA, 
								C0_OC, C0_FACTURA) 
								VALUES('$numero', '$articulo','$cambio_articulo', '$descripcion', '$origen', 01 , $cantidad, '$ftraspaso', '$hoy', $recno, ' ',
								'$user','40','$um','$cod_barra',$conv,'$segum',$recno_traspaso,'$conta','$grupo',$custo1_save,$custo5_save,'0',$cant2um, $cantidad_entrada, '$cantidad_tipo',
								(SELECT LPAD(MAX(C0_NUM) + 1, 6, 0) as N_RESERVA  FROM SC0020),
								(SELECT C0_OC FROM ZC0020 WHERE C0_RESERVA = '$origen'),
								(SELECT C0_OC FROM ZC0020 WHERE C0_RESERVA = '$origen'))";
...
function reserva_mala_os($bodega_origen,$articulo){
	global $tipobd_totvs,$conexion_totvs;
		//borrar la reserva
		$queryup_1 = "UPDATE SC0020 SET D_E_L_E_T_ = '*', C0_CODCLAS = 'NC', C0_CLASIF = 'RPNC' , R_E_C_D_E_L_ = (SELECT R_E_C_N_O_ FROM SC0020  WHERE C0_NUM = '".$bodega_origen."' and C0_PRODUTO='".$articulo."')
		WHERE C0_NUM = '".$bodega_origen."' and C0_PRODUTO='".$articulo."' ";
		//echo $queryup_1;
		$rsu_1 = querys($queryup_1, $tipobd_totvs, $conexion_totvs);

		echo "reserva '".$bodega_origen."'con NParte ".$articulo." con estado Dañada";
		echo "<br>";
}
function delete_reserva_buena($bodega_origen,$articulo){
	global $tipobd_totvs,$conexion_totvs;
		//borrar la reserva
		$queryup_1 = "UPDATE SC0020 SET D_E_L_E_T_ = '*', C0_CODCLAS = 'RB', R_E_C_D_E_L_ = (SELECT R_E_C_N_O_ FROM SC0020  WHERE C0_NUM = '".$bodega_origen."' and C0_PRODUTO='".$articulo."')
		WHERE C0_NUM = '".$bodega_origen."' and C0_PRODUTO='".$articulo."' ";
		//echo $queryup_1;
		$rsu_1 = querys($queryup_1, $tipobd_totvs, $conexion_totvs);
...
function ver_traspasos(){
	global $tipobd_totvs,$conexion_totvs;
		...
		$querysel = "SELECT SC.C0_NUM, SC.C0_OS, SC.C0_SOLICIT, SC.C0_PRODUTO, SC.C0_QUANT, SC.C0_CLASIF, SC.C0_CODCLAS, 
					SC.D_E_L_E_T_,SC.R_E_C_D_E_L_, SC.R_E_C_N_O_, SC.C0_RESANT, SC.C0_EMISSAO, SC.C0_VALIDA, SC.C0_OC, SC.C0_FACTURA
					FROM SC0020 SC
					WHERE SC.C0_TABORI ='SD1' AND  SC.D_E_L_E_T_ <> '*'";
	...
	while($v = ver_result($rss, $tipobd_totvs)){
		...
		$cargar[]=array(
					...
					"CANTIDAD"	=>$v["C0_QUANT"],
					"OC"		=>$v["C0_OC"],
					"INVOICE"	=>$v["C0_FACTURA"],
					"FEMISION"	=>formatDate($v["C0_EMISSAO"]),
					"FVALIDA"	=>$v["C0_EMISSAO"],
					"ESTADO"	=>$v["C0_CODCLAS"],
					"ESTADO_1"	=>$v["C0_CLASIF"],
					"DOC"		=>$v["C0_DOC"],
...
function repuestos_malos($articulo, $cantidad, $bodega_origen){
	...
	$querys_estado = "SELECT C0_OS AS OS FROM SC0020 WHERE C0_PRODUTO='$articulo' AND C0_NUM='$bodega_origen'";
    // echo $querysel;
    $rss = querys($querys_estado, $tipobd_totvs, $conexion_totvs);
	$v1 = ver_result($rss, $tipobd_totvs);
	$os =  $v1["OS"];
	...
	VALUES(	'01', ' ', ' ', (SELECT LPAD(MAX(C0_NUM) + 1, 6, 0) as N_RESERVA  FROM SC0020), '$os', 'VD', ' ', 'I+D', '01', 
	'$articulo', '01', ' ', $cant_mala, 0, 0, 0, 0,
	0, '$fecha_valida', 0, 0, '$hoy', ' ', ' ', ' ', 
	' ', 0, ' ', ' ',0, 0, 0, 0,
	' ', ' ', '$articulo', '01', ' ', ' ', ' ', ' ',
	(SELECT MAX(R_E_C_N_O_) +1 FROM SC0020), 0, 'RPNC', 'NC', '$bodega_origen',
	(SELECT C0_OC FROM SC0020 WHERE C0_NUM = '$bodega_origen'),
	(SELECT C0_FACTURA FROM SC0020 WHERE C0_NUM = '$bodega_origen'))
	";
...
function repuestos_buenos($articulo, $cantidad, $bodega_origen){
	...
	' ', 0, ' ', ' ',0, 0, 0, 0,
	' ', ' ', '$articulo', '01', ' ', ' ', ' ', '*',
	(SELECT MAX(R_E_C_N_O_) +1 FROM SC0020), (SELECT MAX(R_E_C_N_O_) +1 FROM SC0020), 'INVESTIGACION', 'RB', '$bodega_origen',
	(SELECT C0_OC FROM SC0020 WHERE C0_NUM = '$bodega_origen'),
	(SELECT C0_FACTURA FROM SC0020 WHERE C0_NUM = '$bodega_origen'))";
...
function repuestos_pendientes_os($articulo, $cantidad, $bodega_origen){
	...
	0, '$fecha_valida', 0, 0, 
	(SELECT C0_EMISSAO FROM SC0020 WHERE C0_NUM = '$bodega_origen'),
	 ' ', ' ', ' ', 
	' ', 0, ' ', ' ',0, 0, 0, 0,
	' ', ' ', '$articulo', '01', ' ', ' ', ' ', ' ',
...
function reserva_original($articulo, $cantidad, $bodega_origen){
	...
	global $tipobd_totvs,$conexion_totvs;
			...
			C0_PRODUTO, C0_LOCAL, C0_XUBICA, '$cantidad' , C0_XCONSUM, C0_PENDIEN, C0_XSALDOA, C0_QSOLICI, 
			C0_SALDOAC, C0_VALIDA, C0_QD3DOC, C0_D3QUANT, C0_EMISSAO, C0_NUMLOTE, C0_LOTECTL, C0_LOCALIZ, 
			C0_NUMSERI, C0_QTDPED, C0_OBS, C0_UBICA, C0_QTDELIM, C0_QTDORIG, C0_RESERVA, C0_RECNOCP, 
			C0_NUMREQ, C0_ITEMREQ, C0_CODREQ, C0_FILDES, C0_ORIGEN, C0_TABORI, C0_RECORI, ' ', 
			(SELECT MAX(R_E_C_N_O_) +1 FROM SC0020), 0 , 'INVESTIGACION', 'RB', C0_NUM
			FROM SC0020 WHERE C0_NUM = '".$bodega_origen."' and C0_PRODUTO='".$articulo."' ";
			//echo $query_03;
...
function confirmar_traspaso($numero, $articulo, $bodega_origen, $cantidad, $cantidad_tipo){
	...
	$querys_estado = "SELECT C0_OS AS ESTADO FROM SC0020 WHERE C0_PRODUTO='$articulo' AND C0_NUM='$bodega_origen'";
    $rss = querys($querys_estado, $tipobd_totvs, $conexion_totvs);
	$v1 = ver_result($rss, $tipobd_totvs);
	$estado =  $v1["ESTADO"];
...
if ($estado == 'RPNC01'){
	if($cantidad_tipo == 10){
		...
		if($stock == $cantidad){
		delete_reserva_buena($bodega_origen,$articulo);
	...
	if($cantidad_tipo == 10){
		...
		if($stock == $cantidad){
		delete_reserva_buena($bodega_origen,$articulo);
		reserva_original($articulo, $cantidad, $bodega_origen);
		}elseif($stock > $cantidad){
		delete_reserva($bodega_origen,$articulo);
		reserva_original($articulo, $cantidad, $bodega_origen);
		repuestos_pendientes_os($articulo, $cantidad, $bodega_origen);
		}
	}elseif($cantidad_tipo == 20){
		if ($stock == $cantidad){
			reserva_mala_os($bodega_origen,$articulo);
		}elseif($stock > $cantidad){
			delete_reserva($bodega_origen,$articulo);
			repuestos_malos($articulo, $cantidad, $bodega_origen);
			repuestos_pendientes_os($articulo, $cantidad, $bodega_origen);
...
function get_doc(){
	...
	$query = "SELECT CO_DOC FROM TOTVS12C.ZC0020 WHERE CO_SECUENCIA = '".$_GET['num']."' AND TRIM(CO_COD) =  '".$_GET['articulo']."' ";
	$result = querys($query, $tipobd_totvs, $conexion_totvs); 
...
if(isset($_GET["confirma_traspaso"])){
	...
	send_mail('jesus.cesin@ptt.cl','Jesus Cesin',$_GET["numero"], define_smtp(),4);
