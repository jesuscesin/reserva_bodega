...
function generaInsert($frm){
    ...
    if($r == 0){
      ...
      if(count($asig)>0){//el array $haberes por lo menos siempre tendra el valor de cantidad de filas de la tabla haberes
            ...
            $cantidad   		= $frm["cantidad"];
			$cantidadanalizada = $frm["cantidadanalizada"];
            $hoy         		= date('Ymd');
            $hora		 		= date('His');
            $mes		 		= date('m');
            $ano		 		= date('Y');
...
//    echo"</pre>";
	...
	$recno			= recno_traspaso();
		...
		$cantidadanalizada  = $post["cantidadanalizada"];
        $hoy         		= date('Ymd');
		$hora		 		= date('His');
		$mes		 		= date('m');
		$ano		 		= date('Y');
...
function actualiza_stock($cantidad,$articulo, $bodega_origen){
		...
		echo "reserva '".$bodega_origen."' eliminada";
}
function confirmar_traspaso($numero, $articulo, $bodega_origen, $cantidad, $cantidadanalizada){
	global $tipobd_totvs,$conexion_totvs;
...
/* 	echo $numero;
	...
	if ($stock == $cantidadanalizada){
	
	if($cantidadanalizada == $cantidad){
	...
	elseif()
 	if($cantidadanalizada == $cantidad){
		...
		$new_cant = $stock - $cantidadanalizada;
		...
		R_E_C_N_O_, R_E_C_D_E_L_, C0_CLASIF, C0_CODCLAS) 
		...
		' ', 0, ' ', ' ',0, 0, 0, 0,
		' ', ' ', '$articulo', '01', ' ', ' ', ' ', ' ',
		(SELECT MAX(R_E_C_N_O_) +1 FROM SC0020), 0, 'INVESTIGACION', 'ID')";

		$rsi = querys($queryin, $tipobd_totvs, $conexion_totvs);

		$queryin2 = "INSERT INTO SC0020(C0_FILIAL, C0_NUMSCP, C0_ITEMSCP, C0_NUM, C0_OS, C0_TIPO, C0_DOCRES, C0_SOLICIT, C0_FILRES, 
		C0_PRODUTO, C0_LOCAL, C0_XUBICA, C0_QUANT, C0_XCONSUM, C0_PENDIEN, C0_XSALDOA, C0_QSOLICI, 
		C0_SALDOAC, C0_VALIDA, C0_QD3DOC, C0_D3QUANT, C0_EMISSAO, C0_NUMLOTE, C0_LOTECTL, C0_LOCALIZ, 
		C0_NUMSERI, C0_QTDPED, C0_OBS, C0_UBICA, C0_QTDELIM, C0_QTDORIG, C0_RESERVA, C0_RECNOCP, 
		C0_NUMREQ, C0_ITEMREQ, C0_CODREQ, C0_FILDES, C0_ORIGEN, C0_TABORI, C0_RECORI, D_E_L_E_T_, 
		...
		' ', 0, ' ', ' ',0, 0, 0, 0,
		' ', ' ', '$articulo', '01', ' ', ' ', ' ', '*',
		(SELECT MAX(R_E_C_N_O_) +1 FROM SC0020), (SELECT MAX(R_E_C_N_O_) +1 FROM SC0020), 'INVESTIGACION', 'ID')";

		$rsi = querys($queryin2, $tipobd_totvs, $conexion_totvs);

		echo $queryin2;
		// Obtiene el valor del campo C0_NUM de la nueva reserva
		$query_get_new_reserva = "SELECT C0_NUM AS NEW_RESERVA FROM SC0020 WHERE R_E_C_N_O_ = (SELECT MAX(R_E_C_N_O_) FROM SC0020)";
		$rs_get_new_reserva = querys($query_get_new_reserva, $tipobd_totvs, $conexion_totvs);
		$row_new_reserva = ver_result($rs_get_new_reserva, $tipobd_totvs);
		$new_reserva_num = $row_new_reserva['NEW_RESERVA'];
		//ECHO $query_get_new_reserva;

		// Muestra el mensaje con el número de la nueva reserva
		echo "Se eliminó la reserva $bodega_origen y se creó una nueva reserva con el resto. Nueva reserva número: $new_reserva_num";
		...
		$new_cant = $stock - $cantidadanalizada;
		$cant_mala = $cantidadanalizada - $cantidad;

		//echo $cantidadanalizada;
		...
		' ', 0, ' ', ' ',0, 0, 0, 0,
		' ', ' ', '$articulo', '01', ' ', ' ', ' ', ' ',
		(SELECT MAX(R_E_C_N_O_) +1 FROM SC0020), 0, 'RPNC', 'NC')";
...
//Datos Personales
	...
	$querysel ="SELECT DISTINCT  C0_SECUENCIA,
				C0_FTRASPASO,
				(SELECT ZZ1_BODEGA||'_'||ZZ1_DESCRI FROM ZZ1010 WHERE ZZ1_FILIAL='01' AND ZZ1_BODEGA=C0_ORIGEN) AS C0_ORIGEN,
				(SELECT ZZ1_BODEGA||'_'||ZZ1_DESCRI FROM ZZ1010 WHERE ZZ1_FILIAL='01' AND ZZ1_BODEGA=C0_DESTINO) AS C0_DESTINO,
				C0_USUARIO
				FROM ZC0020
...
if(isset($_GET["confirma_traspaso"])){
	...
	$bodega_origen 	= $_GET['bodega_origen'];
	$cantidad       = $_GET['cantidad'];
	$cantidadanalizada       = $_GET['cantidadanalizada'];
    confirmar_traspaso($numero, $articulo, $bodega_origen,  $cantidad, $cantidadanalizada);
	ver_pedido_impreso($numero, $articulo, $bodega_origen,  $cantidad);
