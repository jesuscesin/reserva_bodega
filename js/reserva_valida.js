...
function valida_traspasook() { 
    ...
    //alert(ntraspaso);
       Swal.fire({
            icon: 'question',
            title: "Desea Validar Reserva <br> # "+id_traspaso+" ?",
...
function anula_traspaso() { 
    ...
    secuencia = $(this).attr('id_traspaso');
       Swal.fire({
            icon: 'question',
            title: "Desea Anular Solicitud # "+secuencia+" ?",
            text: "Porceso de Solicitud Quedara Anulado",
            width: '500px',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
        ...
        success:function(data){
            $("#mensajes").html("<div class='alert alert-success' role='alert'>"+data+"</div>");
            $("#"+secuencia).remove();
            Swal.fire('Reserva Anulada !', data, 'info');
        },
            //error:problemas
        });
        } else {
            ...
            $.each(jsonphp, function(indice,valores){
                    ...
                    var tr = $("<tr />").attr('id',valores.ID);
                     if (valores.ESTADO === '20') {
                              ...
                              tr.attr('style','background-color:  #83D2F7');                   
                               ...
                               tr.attr('style','bg-warning color-palette');                 
                     }
                    ...
                    if(valores.ESTADO==10){
                        $('<td />').html('PROCESANDO  <i class="fas fa-spinner"></i>' ).appendTo(tr);
                         ...
                         $('<td />').html("POR VALIDAR <img src='img/icon/check.png'></img>").appendTo(tr);     
                    }else if(valores.ESTADO==30){
                    ...
                    if(valores.DOC == null){
                        ...
                        $("<a>").attr({'href': 'r_docs/' + valores.DOC, 'target': '_blank','title':valores.DOC,'class':'btn btn-block'}).html('<i class="fa-solid fa-file-lines"></i>').appendTo($("<td>").appendTo(tr));
                    }
