...
function ver_traspasos($bodega_origen,$bodega_destino,$status){
	...
	if($status == 'todos'){
		$querysel = "SELECT C0_SECUENCIA,
					...
					FROM ZC0020 B
					WHERE  (LENGTH(C0_ORIGEN)  = '$bodega_origen'
		...
		$querysel = "SELECT C0_SECUENCIA,
					(SELECT ZZ1_BODEGA||'_'||ZZ1_DESCRI FROM ZZ1020 WHERE ZZ1_FILIAL='01' AND ZZ1_BODEGA=C0_ORIGEN) AS C0_ORIGEN,
					(SELECT ZZ1_BODEGA||'_'||ZZ1_DESCRI FROM ZZ1020 WHERE ZZ1_FILIAL='01' AND ZZ1_BODEGA=C0_DESTINO) AS C0_DESTINO,
					COUNT(C0_COD) AS ARTICULOS,
                    SUM(C0_CANTIDAD) AS CANTIDAD,
                    SUM(C0_CANTEN) AS CANTIDAD_ENTRADA,
					C0_FTRASPASO,C0_STATUS, NVL((SELECT DISTINCT D3_ESTORNO FROM SD3010 WHERE REPLACE(C0_SECUENCIA,'-','')=D3_DOC),' ') AS D3_ESTORNO
					FROM ZC0020 B
					WHERE  (C0_ORIGEN  like '%$bodega_origen%'
					AND C0_DESTINO  like '%$bodega_destino%'
					AND C0_STATUS = '$status')
                    AND B.D_E_L_E_T_<>'*'
					GROUP BY C0_SECUENCIA,C0_ORIGEN,C0_DESTINO,C0_FTRASPASO,C0_STATUS
					ORDER BY substr(C0_SECUENCIA,4,6) DESC";//,substr(C0_SECUENCIA,-2),C0_FECHA
					echo $querysel;
	}
...
function valida_reserva($numero){
	...
	$querysel = "SELECT *
		...
		while($v = ver_result($rss, $tipobd_totvs)){
			...
			$queryup_1 = "UPDATE SC0020 SET D_E_L_E_T_ = '*' WHERE C0_NUM = '".$filial."' and C0_PRODUTO='".$np_cod."' ";
			...
			(SELECT MAX(R_E_C_N_O_) +1 FROM SC0020), R_E_C_D_E_L_, C0_USERLGI, C0_USERLGA 
			...
			C0_NUMREQ, C0_ITEMREQ, C0_CODREQ, C0_FILDES, C0_ORIGEN, C0_TABORI, C0_RECORI, D_E_L_E_T_, 
			R_E_C_N_O_, R_E_C_D_E_L_, C0_USERLGI, C0_USERLGA)
			SELECT C0_FILIAL, C0_NUMSCP, C0_ITEMSCP, (SELECT LPAD(MAX(C0_NUM) +1,6,0) FROM SC0020), 'I+D', C0_TIPO, C0_DOCRES, C0_SOLICIT, C0_FILRES, 
