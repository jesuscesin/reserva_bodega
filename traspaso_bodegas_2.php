...
function cargar_stock($articulo,$bodega_origen){
	...
	$querysel = "SELECT (B2_QATU - B2_RESERVA) AS CANTIDAD FROM SB2020 WHERE B2_COD='$articulo' AND B2_LOCAL='$bodega_origen' AND B2_FILIAL = '01'";
					//echo $querysel;
	$rss = querys($querysel, $tipobd_totvs, $conexion_totvs);
	while($v = ver_result($rss, $tipobd_totvs)){
		$csk[]=array(
...
function get_secuencia($bodega_origen){
	...
	$querysel = "SELECT NVL(SUBSTR(MAX(BO_SECUENCIA),4,7)+1,1) AS BO_TRASPASO
					FROM ZD3020 where BO_ORIGEN ='$bodega_origen'";
	//echo $querysel;
	$rss = querys($querysel, $tipobd_totvs, $conexion_totvs);
	while($fila = ver_result($rss, $tipobd_totvs)){
...
function getrecno_secuencia($bodega_origen){
	...
	$querysel = "SELECT NVL(MAX(BO_RSECUENCIA),0)+1 AS CORRELATIVO FROM ZD3020 WHERE BO_ORIGEN='$bodega_origen'";
	//echo $querysel;
	$rss = querys($querysel, $tipobd_totvs, $conexion_totvs);
	while($fila = ver_result($rss, $tipobd_totvs)){
...
function generaInsert($frm){
    ...
    if($r == 0){
      ...
      if(count($asig)>0){//el array $haberes por lo menos siempre tendra el valor de cantidad de filas de la tabla haberes
			...
			if($existe_codigo == 0){
				$queryin = "INSERT INTO ZD3020(BO_SECUENCIA, BO_COD,BO_CAMARTICULO, BO_DESCR, BO_ORIGEN, BO_DESTINO, BO_CANTIDAD, BO_FTRASPASO, BO_FECHA, R_E_C_N_O_, D_E_L_E_T_,
								BO_USUARIO,BO_STATUS,BO_UM,BO_CODBAR,BO_CONV,BO_SEGUM,BO_RSECUENCIA,BO_CONTA,BO_GRUPO,BO_CUSTO1,BO_CUSTO5,BO_NUMSEQ,BO_CANT2UM, BO_CANTEN, BO_ORIGEN_SOLICITUD) 
								VALUES('$numero', '$articulo','$cambio_articulo', '$descripcion', '$origen', '$destino', $cantidad, '$ftraspaso', '$hoy', $recno, ' ',
								'$user','10','$um','$cod_barra',$conv,'$segum',$recno_traspaso,'$conta','$grupo',$custo1_save,$custo5_save,'0',$cant2um, $cantidad_entrada,'$origen_sol')";
...
function existe_codigo($articulo,$cambio_articulo,$numero){
	...
	$querycount = "SELECT count(*) as FILAS FROM ZD3020 where BO_COD='$articulo' and BO_CAMARTICULO='$cambio_articulo' and BO_SECUENCIA='$numero' and D_E_L_E_T_<>'*'";
...
function eliminar_articulo($articulo,$numero){
	...
	$queryup ="UPDATE ZD3020 SET D_E_L_E_T_='*' WHERE BO_COD='$articulo' AND BO_SECUENCIA='$numero'";///PREGUHTAR ESTADOS
...
function revisa_confirmacion($numero){
	...
	$querysel = "SELECT BO_COD,BO_CAMARTICULO FROM ZD3020 WHERE BO_SECUENCIA='$numero' AND D_E_L_E_T_<>'*'";
	$rss = querys($querysel, $tipobd_totvs, $conexion_totvs);
	while($v = ver_result($rss, $tipobd_totvs)){
		$cod_salida = $v["BO_COD"];
...
function confirmar_traspaso($numero){
	...
	$queryup ="UPDATE ZD3020 SET BO_STATUS='20' WHERE  BO_SECUENCIA='$numero'";///PREGUHTAR ESTADOS
	$rsu =querys($queryup, $tipobd_totvs, $conexion_totvs);
	//echo $queryup;
	if(oci_num_rows($rsu)<>0 or oci_num_rows($rsu)<>null){
			echo "TRASPASO CONFIRMADO ! <br> <br> <br> <a target='_blank' class='btn btn-danger' href='solicitud_transferencia.php?id=$numero'><strong>Descarga Aqui Ficha de Traspaso</a></strong> ";
		}else{
...
function count_filas($numero){
	...
	$select = "SELECT COUNT(*) AS FILAS FROM ZD3020 WHERE BO_SECUENCIA='$numero'";
...
function recno_traspaso(){
	...
	$select = "SELECT NVL(MAX(R_E_C_N_O_),0)+1 AS CORRELATIVO FROM ZD3020";
	$rs = querys($select, $tipobd_totvs, $conexion_totvs);
...
function recno_secuencia_s($bodega){
	...
	$select = "SELECT NVL(MAX(BO_RSECUENCIA),0)+1 AS CORRELATIVO FROM ZD3020 WHERE BO_ORIGEN='$bodega' ";
	$rs = querys($select, $tipobd_totvs, $conexion_totvs);
	$fila = ver_result($rs, $tipobd_totvs);
	$recno = $fila['CORRELATIVO'];
	return $recno;
...
function recno_secuencia(){
	...
	$querysel = "SELECT MAX(R_E_C_N_O_)+1 AS RECNO FROM ZD3020";
	$rss = querys($querysel, $tipobd_totvs, $conexion_totvs);
	while($v = ver_result($rss, $tipobd_totvs)){
			$fec[] = array(
			'RECNO' => $v['RECNO']
...
function confirmacion_correo($numero){
	...
	$s = "SELECT BO_DESTINO FROM ZD3020 
			WHERE BO_SECUENCIA='$numero'
			GROUP BY BO_DESTINO";
...
function subirArchivo(){
    ...
    if($_FILES['file_doc']["name"] != NULL){
	    ...
	    move_uploaded_file($_FILES['file_doc']["tmp_name"],'docs/'.$updfiles);

		$query = "UPDATE ZD3020 SET BO_DOC = '".$updfiles."' WHERE BO_SECUENCIA = '".$_GET['num']."' AND TRIM(BO_COD) =  '".$_GET['np']."' ";
...
function get_doc(){
	...
	$query = "SELECT BO_DOC FROM ZD3020 WHERE BO_SECUENCIA = '".$_GET['num']."' AND BO_COD =  '".$_GET['articulo']."' ";
	$result = querys($query, $tipobd_totvs, $conexion_totvs); 
...
function get_doc(){
	...
	$query = "SELECT BO_DOC FROM TOTVS12C.ZD3020 WHERE BO_SECUENCIA = '".$_GET['num']."' AND TRIM(BO_COD) =  '".$_GET['articulo']."' ";
	$result = querys($query, $tipobd_totvs, $conexion_totvs); 
...
if(isset($_GET["confima_traspaso"])){
	...
	enviar_notificacion('carlos.escobar@turbodal.cl','Carlos Escobar',$numero);
	//enviar_notificacion('romina.defilippi@ptt.cl','Romina Defilippi',$numero);
